...
use crate::{
    ...
    set::model::{MergedSetInputData, SetModel, SetToCreate},
    setup::get_pool,
...
pub async fn exercise_log_bulk_create(
    ...
    let exercises =
        ExerciseModel::bulk_create_from_exercise_plan(&pool, workout_id, &exercise_plans, user.id)
            .await?;
    let merged_data = MergedSetInputData::merge_from_exercise_sets(&exercises, &sets_to_create);
    SetModel::bulk_create_from_set_input_data_vec(&pool, &merged_data, user.id).await?;
    Ok(())
}
