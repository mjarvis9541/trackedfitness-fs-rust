...
pub async fn get_request_user_stats_detail() -> Result<UserStat, ServerFnError> {
    let user = get_request_user()?;
    let pool = get_pool()?;
    let query = UserStat::get_object_or_404(&pool, &user.username).await?;
    Ok(query)
}
...
#[component]
pub fn UserStatDetailComponent(data: UserStat) -> impl IntoView {
    view! {
