...
#[server(endpoint = "user-block-list")]
...
pub async fn create_user_block(username: String) -> Result<(), ServerFnError> {
    ...
    if results == 0 {
        return Err(ServerFnError::new("Error blocking user"))?;
    }
    Ok(())
}
...
pub async fn update_user_block(username: String) -> Result<(), ServerFnError> {
    ...
    let pool = get_pool()?;
    if user.username == username {
        return Err(ServerFnError::new("You cannot unblock yourself"));
    }
    ...
    if results == 0 {
        return Err(ServerFnError::new("Error ublocking user"))?;
    }
    Ok(())
}
    ...
    view! {
        <div class="flex items-start p-2 mb-4 bg-gray-100">
            ...
            <div class="flex gap-2">
                <UserUnblockButton blocked_username=blocked_username action/>
            </div>
        </div>
    }
}
