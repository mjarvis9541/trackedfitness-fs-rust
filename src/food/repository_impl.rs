...
impl Food {
    pub async fn get_by_slug(pool: &PgPool, slug: &str) -> Result<Option<Self>> {
        let query = sqlx::query_as!(Self, "SELECT * FROM food WHERE slug = $1", slug)
            .fetch_optional(pool)
            .await?;
        Ok(query)
    }
    ...
    pub async fn delete(pool: &PgPool, id: Uuid) -> Result<Self> {
        let query = sqlx::query_as!(Self, "DELETE FROM food WHERE id = $1 RETURNING *", id)
            .fetch_one(pool)
            .await?;
        Ok(query)
    }
...
impl FoodQuery {
    ...
    pub async fn filter(
        ...
        let order_by_column = get_order_by_column(order);
        let init = "
            SELECT
                t1.*,
                t2.name as brand_name,
...
impl FromRow<'_, PgRow> for FoodQuery {
