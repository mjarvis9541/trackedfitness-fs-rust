...
impl Follower {
    ...
    pub async fn try_get(pool: &PgPool, id: Uuid) -> sqlx::Result<Option<Self>> {
        sqlx::query_as!(
            Self,
            r#"
            SELECT t1.*, followed_user.username as "username!", follower_user.username as "follower!"
            FROM user_follower t1 
    ...
    pub async fn get_object_or_404(pool: &PgPool, id: Uuid) -> Result<Self> {
        let object = Self::try_get(pool, id).await?.ok_or(Error::NotFound)?;
        Ok(object)
    }
    ...
    pub async fn create(
        ...
        let query = sqlx::query(
            "
                INSERT INTO user_follower (user_id, follower_id, status)
                VALUES ($1, $2, $3)
            ",
        )
        .bind(user_id)
    ...
    pub async fn count(
        ...
        qb_scalar.filter("t2.username", "=", username);
        qb_scalar.filter("t3.username", "=", follower);
        if !status.is_empty() {
            let status = status.parse::<i64>().unwrap_or(0);
            qb_scalar.push(" AND t1.status = ");
            qb_scalar.push_bind(status);
        };
        let count = qb_scalar.build_query_scalar().fetch_one(pool).await?;
        Ok(count)
    }
    ...
    pub async fn filter(
        ...
        let mut qb = sqlx::QueryBuilder::new(
            "
                SELECT t1.*, t2.username as username, t3.username as follower
                FROM user_follower t1
                LEFT JOIN users_user t2 ON t2.id = t1.user_id
                LEFT JOIN users_user t3 ON t3.id = t1.follower_id
                WHERE TRUE
