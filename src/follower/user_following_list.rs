...
use crate::follower::form::{FollowerAccept, FollowerUnfollow, FollowerUnfollowForm};
...
#[server]
pub async fn get_request_user_following_list(
    search: String,
    order: String,
    size: i64,
    page: i64,
...
#[component]
pub fn CurrentUserFollowingList() -> impl IntoView {
    ...
    let resource = Resource::new(
        ...
        |(search, order, size, page, _, _)| {
            get_request_user_following_list(search, order, size, page)
        },
    );
    let response = move || {
    ...
    view! {
        <header class="mb-4">
            <h2 class="mb-2 text-base font-bold">
                "Following" <Transition fallback=Loading>" (" {count} ")"</Transition>
            </h2>
