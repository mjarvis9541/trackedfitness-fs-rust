...
FROM
    ...
    LEFT JOIN (
        SELECT
            user_id,
            COUNT(DISTINCT date) AS diet_day_count
        FROM
            food_log
        GROUP BY
            user_id
    ...
    LEFT JOIN (
        SELECT
            created_by_id,
            COUNT(*) AS movement_created_count
        FROM
            movement
        GROUP BY
            created_by_id
    ...
    LEFT JOIN (
        SELECT
            created_by_id,
            COUNT(*) AS muscle_group_created_count
        FROM
            muscle_group
        GROUP BY
            created_by_id
    ...
    LEFT JOIN (
        SELECT
            created_by_id,
            COUNT(*) AS meal_of_day_created_count
        FROM
            meal_of_day
        GROUP BY
            created_by_id
    ...
    LEFT JOIN (
        SELECT
            f2.user_id,
            COUNT(f1.id) AS meal_food_created_count
        FROM
            meal_food f1
    ...
    LEFT JOIN (
        SELECT
            user_id,
            COUNT(DISTINCT date) AS workout_day_count
        FROM
            workout
        GROUP BY
            user_id
